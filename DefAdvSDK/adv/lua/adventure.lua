...
function M.addcondition(self)
	if M.conditionalpos>0 then
		if M.conditionals==nil then
			M.conditionals={}
		end
		M.conditionals[M.conditionalpos]=M.conditional
	end
				...
				elseif cmd=="endif" then				
					if M.conditionalcheck==M.conditionalpos then
						...
						if M.conditionalpos==0 then
							...
							M.conditional=M.conditionals[M.conditionalpos]
						end
					else	
						M.conditionalpos=M.conditionalpos-1
						if M.conditionalpos==0 then
					...
					if M.conditionalpos~=M.conditionalcheck or (M.conditional and M.conditional==0) then
						...
						elseif cmd=="moveto" then
							...
							local pos = vmath.vector3(dig[1],screen_h-((dig[2]-herosize.y/2)),0)
							msg.post(M.player, "move_to",{destination=pos,follow=true})			
							M.waitfor=M.WAITFORMOVEMENT -- heroendmovement										
...
function M.reset(self)
	...
	for i = 1, M.inventory.count do
		msg.post("hud", "hud_setinv",{num=i,val=0,img=M.inventory.blankicon})
	end
...
function M.handle_onclick(self,action,right)
	...
	if M.commands then
		if action.pressed == true then
			if M.waitfor==M.WAITFORTALK or M.waitfor==M.WAITFORWAIT then
				M.waittime=0
			end
		end
	else
...
function M.update(self,dt)
	if M.commands then
		if M.waitfor == M.WAITFORMOVEMENT then
			if heroendmovement==true then
				M.playcommands(self)
			end
		elseif M.waitfor == M.WAITFORWAIT then
		...
		elseif M.waitfor == M.WAITFORTALK then
			if M.waittime>0 then
				M.waittime=M.waittime-dt
			end
			if M.waittime<=0 then
